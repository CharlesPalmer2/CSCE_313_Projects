...
void file_thread_function (BoundedBuffer* rb, string filename, TCPRequestChannel* chan, int m) {
    // file size
    filemsg fm(0, 0);
...
void worker_thread_function (BoundedBuffer* rb, BoundedBuffer* resb, TCPRequestChannel* chan, int m) {
    // forever loop
    while(true){
...
int main (int argc, char* argv[]) {
    ...
    string a = "127.0.0.1"; // TCP address
    string r = "";  // TCP port
    ...
    // Commented out as unneeded as server is runned seperate
	// fork and exec the server
    //int pid = fork();
    //if (pid == 0) {
    //    execl("./server", "./server", "-m", (char*) to_string(m).c_str(), "-r", r, nullptr);
    //}
    
	// initialize overhead (including the control channel)
	TCPRequestChannel* chan = new TCPRequestChannel(a, r);
    TCPRequestChannel* f_chan; // For file thread
    ...
    if (f != ""){
        ...
        f_chan = new TCPRequestChannel(a, r);
    ...
    for (int i=0; i<w; i++){
        ...
        TCP.push_back(new TCPRequestChannel(a, r));
        worker_threads.push_back(thread(worker_thread_function, &request_buffer, &response_buffer, TCP.at(i), m));
    }
    // if data:
    if (f == ""){
    ...
    for (int i=0; i<w; i++){
        TCPRequestChannel* TCP_ptr = TCP.at(i);
        (*TCP_ptr).cwrite((char *)&quit, sizeof(MESSAGE_TYPE));
        delete TCP.at(i);
    }
    if (f != ""){
